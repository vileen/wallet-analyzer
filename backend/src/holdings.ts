...
// ── DB: get first snapshot of the day (for day-long changes) ───────

export async function getDayStartSnapshot(walletId: number): Promise<HoldingSnapshot | null> {
  const snapshotResult = await query(
    `SELECT id, wallet_id, snapshot_at, total_value_usd, sol_balance
     FROM holdings_snapshots
     WHERE wallet_id = $1 AND snapshot_at >= DATE_TRUNC('day', NOW())
